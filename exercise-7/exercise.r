...
# Boston Celtics have the lowest ratio 
...
team.data <- mutate(team.data, APG = AST / G) %>%
                    arrange(-APG)
...
BetterShooters <- function(team.1, team.2) {
  better.team <- filter(team.data, Team %in% c(team.1, team.2)) %>%
                        filter(FG. == max(FG.)) %>%
                        select(Team, FG., PTS)
  return(better.team)
}

BetterShooters('Sacramento Kings', 'Chicago Bulls')
